    ...
    <section className="py-14 md:py-16 border-y border-border bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <div className="text-overline mb-2">Indexed & Registered With</div>
          <h2 className="text-h2 text-foreground">Discoverable across the scholarly web</h2>
        </div>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-6 md:gap-8 items-center max-w-4xl mx-auto">
          {INDEXES.map((idx) => (
            <a
              ...
              title={idx.name}
            >
              <span className="font-serif text-h5 !text-muted-foreground group-hover:!text-primary transition-colors text-center">
                {idx.name}
              </span>
            </a>
          ))}
